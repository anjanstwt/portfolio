...
export default function SectionHeading({ className, title, extra }: { className?: string; title: string, extra?: string }) {
    return (
        <div className={cn("relative -mx-4 sm:-mx-6 w-[calc(100%+2rem)] sm:w-[calc(100%+3rem)] mb-4 mt-2", className)}>
            ...
            <div className="absolute left-4 sm:left-6 -top-2.5 z-10 bg-[#09090b] border border-white/20 rounded-[4px] px-3 text-xs font-semibold uppercase tracking-widest text-neutral-400">
                {title}
            </div>
            {extra && (
                <div className="absolute right-4 sm:right-6 -bottom-2.5 z-10 bg-[#09090b] border border-white/20 rounded-[4px] px-3 text-xs font-semibold tracking-widest text-neutral-400">
                    {extra}
                </div>
            )}
        </div>
